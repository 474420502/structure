...
func (tree *Tree[KEY, VALUE]) Add(key KEY, value VALUE) bool {
	target, isExists, _ := tree.put(tree.Center, 1, key)
	if !isExists {
		target.Value = value
		tree.size += 1
...
// Contains
func (set *Tree[KEY, VALUE]) Contains(item KEY) bool {
	if _, ok := set.Get(item); ok {
		return true
	}
	return false
}
...
func (set *Tree[KEY, VALUE]) Empty() bool {
	return set.Size() == 0
}
...
func (tree *Tree[KEY, VALUE]) str(root *Node[KEY, VALUE], content *bytes.Buffer) {
	if root == nil {
		return
	}

	tree.str(root.Children[0], content)
	fmt.Fprintf(content, "%v ", root.Value)
	tree.str(root.Children[1], content)
}
